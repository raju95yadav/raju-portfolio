...
const Navbar = ({ theme, toggleTheme }) => {
  ...
  const navLinks = [
    ...
    { label: 'Contact', id: 'contact', isCta: true },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <a 
            ...
            className="text-xl sm:text-2xl font-extrabold tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-violet-600 via-indigo-600 to-purple-600 dark:from-violet-400 dark:via-indigo-400 dark:to-purple-400 font-sans cursor-pointer hover:opacity-90 transition-opacity"
            title="Return to top"
          >
          ...
          {/* Desktop Nav */}
          <div className="hidden lg:flex items-center space-x-1.5 bg-slate-100/80 dark:bg-slate-900/80 p-1.5 rounded-2xl border border-slate-200/60 dark:border-slate-800/80 backdrop-blur-md">
            {navLinks.map((link) => {
              if (link.isCta) {
                return (
                  <a
                    ...
                    className="ml-1 px-4 py-1.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-500 rounded-xl shadow-md shadow-indigo-600/20 hover:scale-105 active:scale-95 transition-all"
                  >
                    {link.label}
                  </a>
                );
            ...
            {navLinks.map((link) => (
              <a
                ...
                className={`block px-4 py-2.5 text-sm font-semibold rounded-xl transition-all ${
                  link.isCta
                    ? 'text-center text-white bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-600/20 font-bold mt-2'
                    : activeSection === link.id
