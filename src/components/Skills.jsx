...
import { Code, Layout, Database, Wrench, Sparkles, Search, Grid, BarChart2, Award, CheckCircle, GraduationCap, Eye, ShieldCheck, CheckCircle2, ZoomIn, ZoomOut, FileText, Image, X } from 'lucide-react';
...
const Skills = () => {
  ...
  const [activeCertTab, setActiveCertTab] = useState('certificate'); // 'certificate' | 'details'
  ...
  return (
    <section id="skills" ref={sectionRef} className="py-10 relative">
      ...
      {/* Integrated Certifications & Verified Credentials Subsection */}
      <div id="certificates" className="mt-20 pt-12 border-t border-slate-200/60 dark:border-slate-800/60">
          ...
          <div className="relative bg-white dark:bg-slate-900 max-w-4xl w-full rounded-3xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-800 z-10 flex flex-col max-h-[94vh]">
            <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/90 dark:bg-slate-900/90 backdrop-blur-md sticky top-0 z-20">
              ...
              <div className="flex items-center gap-2">
                ...
                {activeCertTab === 'certificate' && (
                  <button
                    onClick={() => setIsCertZoomed(!isCertZoomed)}
                    ...
                    title={isCertZoomed ? "Reset Zoom" : "Zoom Certificate"}
                  >
                    {isCertZoomed ? <ZoomOut size={16} /> : <ZoomIn size={16} />}
                    <span className="hidden sm:inline">{isCertZoomed ? "Fit Screen" : "Zoom In"}</span>
                  </button>
                )}
              ...
              <div className="p-3 sm:p-5 overflow-y-auto flex-1 flex flex-col items-center justify-center bg-slate-950/10 dark:bg-slate-950/60 custom-scrollbar min-h-[55vh]">
                <div 
                  className={`relative rounded-2xl overflow-hidden shadow-2xl border border-slate-200/80 dark:border-slate-700/80 bg-white transition-all duration-300 cursor-pointer flex items-center justify-center p-2 ${
                    isCertZoomed ? 'w-full max-w-4xl' : 'max-w-full'
                  }`}
                  onClick={() => setIsCertZoomed(!isCertZoomed)}
                  title={isCertZoomed ? "Click to fit screen" : "Click to zoom in"}
                >
                  <img 
                    src={nitCertImg} 
